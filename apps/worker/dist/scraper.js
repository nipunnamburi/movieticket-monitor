...
export async function fetchBmsShows(url) {
...
async function fetchViaBrowser(url) {
    ...
    try {
        ...
        const pageState = await page.evaluate(() => {
            return window.__INITIAL_STATE__ || window.__NEXT_DATA__?.props?.pageProps;
        });
        if (pageState) {
...
function parseBmsApiResponse(data) {
    const result = {};
...
function classifyShowStatus(showItem) {
