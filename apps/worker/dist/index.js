...
import { sendTelegramAlert, sendDiscordAlert } from './notifiers.js';
...
// ── BMS Poll Worker ───────────────────────────────────────────────────────────
export const pollWorker = new Worker('bms-poll', async (job) => {
    ...
    const { shows, error } = await fetchBmsShows(monitor.url);
    if (error) {
    ...
    const oldSnapshot = monitor.snapshot || {};
    ...
    if (openings.length > 0) {
        ...
        const channelsUsed = [];
        // 1. Telegram Alert
        const telegramChatId = monitor.telegramChatId || process.env.DEFAULT_TELEGRAM_CHAT_ID;
        if (telegramChatId) {
            const sent = await sendTelegramAlert(telegramChatId, monitor.name, monitor.city, monitor.url, openings);
            if (sent)
                channelsUsed.push('TELEGRAM');
        }
        // 2. Discord Alert
        const discordWebhookUrl = monitor.discordWebhookUrl || process.env.DEFAULT_DISCORD_WEBHOOK_URL;
        if (discordWebhookUrl) {
            const sent = await sendDiscordAlert(discordWebhookUrl, monitor.name, monitor.city, monitor.url, openings);
            if (sent)
                channelsUsed.push('DISCORD');
        }
        // Record alert log
...
export const alertWorker = new Worker('bms-alert', async (job) => {
    const { channel, target, payload } = job.data;
    console.log(`[AlertWorker] Dispatching alert to ${channel} -> ${target}`);
    if (channel === 'TELEGRAM') {
        return await sendTelegramAlert(target, payload.monitorName, payload.city, payload.url, payload.openings);
    }
    if (channel === 'DISCORD') {
        return await sendDiscordAlert(target, payload.monitorName, payload.city, payload.url, payload.openings);
    }
    return { error: `Unsupported channel: ${channel}` };
}, { connection: redis });
function applyFilters(shows, monitor) {
    const filtered = {};
    ...
    for (const [date, theatres] of Object.entries(shows)) {
        if (Array.isArray(filterDates) && filterDates.length > 0) {
            const dateMatch = filterDates.some((fd) => date.toLowerCase().includes(fd.toLowerCase()));
            if (!dateMatch)
                continue;
        }
        filtered[date] = {};
        for (const [theatre, showtimes] of Object.entries(theatres)) {
            if (Array.isArray(filterTheatres) && filterTheatres.length > 0) {
                const theatreMatch = filterTheatres.some((ft) => theatre.toLowerCase().includes(ft.toLowerCase()));
                if (!theatreMatch)
                    continue;
            }
...
function isWithinTimeWindow(timeStr, from, to) {
    if (!from && !to)
        return true;
    ...
    if (from) {
        ...
        if (fromMins !== null && minutes < fromMins)
            return false;
    }
    if (to) {
        const toMins = parseTimeToMinutes(to);
        if (toMins !== null && minutes > toMins)
            return false;
    }
    return true;
}
...
function countTotalShows(shows) {
    let count = 0;
